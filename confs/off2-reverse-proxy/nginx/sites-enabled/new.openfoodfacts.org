...
limit_req_zone $binary_remote_addr zone=off-new:10m rate=240r/m;


server {
    if ($host = static.new.openfoodfacts.org) {
        return 301 https://$host$request_uri;
    } # managed by Certbot


    if ($host = images.new.openfoodfacts.org) {
        return 301 https://$host$request_uri;
    } # managed by Certbot


    if ($host = world.new.openfoodfacts.org) {
        return 301 https://$host$request_uri;
    } # managed by Certbot


    if ($host = new.openfoodfacts.org) {
        return 301 https://$host$request_uri;
    } # managed by Certbot
    ...
    server_name new.openfoodfacts.org world.new.openfoodfacts.org images.new.openfoodfacts.org static.new.openfoodfacts.org;
...
server {
    ...
    # SSL/TLS settings
    ssl_certificate /etc/letsencrypt/live/new.openfoodfacts.org/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/new.openfoodfacts.org/privkey.pem; # managed by Certbot
    ssl_trusted_certificate /etc/letsencrypt/live/new.openfoodfacts.org/chain.pem;
    ...
    # basic auth until it's production
    ...
    # logs location
    access_log  /var/log/nginx/new.openfoodfacts.org.log  main ; # buffer=256K flush=1s;
    error_log   /var/log/nginx/new.openfoodfacts.org.errors.log;
	...
	# WE ARE REDIRECTING TO OPFF because it is a temporary trick to test keycloak
...
#        proxy_cache_valid any 5s;
	# Adds an X-Cache-Status HTTP header in responses to clients: helps debugging the
        ...
        add_header X-Cache-Status $upstream_cache_status;  
        # require basic auth until it's production
	...
	# proxy_buffering off disables caching
 	...
 	proxy_intercept_errors on;
        ...
        # Rate limiting
        limit_req zone=off-new burst=120 nodelay; # burst=120 means you can do 240+120 requests
