...
limit_req_zone $binary_remote_addr zone=obf-new:10m rate=240r/m;
...
server {
    ...
    server_name new.openbeautyfacts.org *.new.openbeautyfacts.org;
...
server {
    ...
    # SSL/TLS settings
    ssl_certificate /etc/letsencrypt/live/new.openbeautyfacts.org/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/new.openbeautyfacts.org/privkey.pem;
    ssl_trusted_certificate /etc/letsencrypt/live/new.openbeautyfacts.org/chain.pem;
    ...
    # logs location
    access_log  /var/log/nginx/new.openbeautyfacts.org.log  main buffer=256K flush=1s;
    error_log   /var/log/nginx/new.openbeautyfacts.org.errors.log;
...
#        proxy_cache_valid any 5s;
	...
	# proxy_buffering off disables caching
 	...
 	proxy_intercept_errors on;
        ...
        # Rate limiting
        limit_req zone=obf-new burst=120 nodelay; # burst=120 means you can do 240+120 requests
