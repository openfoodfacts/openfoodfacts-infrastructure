...
limit_req_zone $binary_remote_addr zone=opf-new:10m rate=240r/m;
...
server {
    ...
    server_name new.openproductsfacts.org *.new.openproductsfacts.org;
...
server {
    ...
    # SSL/TLS settings
    ssl_certificate /etc/letsencrypt/live/new.openproductsfacts.org/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/new.openproductsfacts.org/privkey.pem;
    ssl_trusted_certificate /etc/letsencrypt/live/new.openproductsfacts.org/chain.pem;
    ...
    # logs location
    access_log  /var/log/nginx/new.openproductsfacts.org.log  main buffer=256K flush=1s;
    error_log   /var/log/nginx/new.openproductsfacts.org.errors.log;
...
#        proxy_cache_valid any 5s;
	...
	# proxy_buffering off disables caching
 	...
 	proxy_intercept_errors on;
        ...
        # Rate limiting
        limit_req zone=opf-new burst=120 nodelay; # burst=120 means you can do 240+120 requests
