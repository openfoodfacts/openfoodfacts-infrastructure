...
limit_req_zone $binary_remote_addr zone=opff-new:10m rate=240r/m;
...
server {
    ...
    server_name new.openpetfoodfacts.org *.new.openpetfoodfacts.org;
...
server {
    ...
    # SSL/TLS settings
    ssl_certificate /etc/letsencrypt/live/new.openpetfoodfacts.org/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/new.openpetfoodfacts.org/privkey.pem;
    ssl_trusted_certificate /etc/letsencrypt/live/new.openpetfoodfacts.org/chain.pem;
    ...
    # logs location
    access_log  /var/log/nginx/new.openpetfoodfacts.org.log  main ; # buffer=256K flush=1s;
    error_log   /var/log/nginx/new.openpetfoodfacts.org.errors.log;
...
#        proxy_cache_valid any 5s;
	...
	# proxy_buffering off disables caching
 	...
 	proxy_intercept_errors on;
        ...
        # Rate limiting
        limit_req zone=opff-new burst=120 nodelay; # burst=120 means you can do 240+120 requests
