...
```javascript
$x("//*[starts-with(@id, 'pmgQueueAdministration')]//table[contains(@class, 'x-grid-item')]//td[contains(@class,'x-grid-cell')][4]/div[contains(@class, 'x-grid-cell-inner') and contains(text(), '@')]/text()")..map(x => x.nodeValue).join("\n")
```
