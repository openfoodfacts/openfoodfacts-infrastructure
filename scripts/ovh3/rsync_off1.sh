...
do
  nice -n 19 ionice -c 3 \
    rsync -a -x -e "ssh -T -x" \
    off@off1.openfoodfacts.org:/srv2/$dir_name $backup_dir/srv2/$dir_name --exclude=html/images/products/ --exclude=orgs/ --exclude=users/ &
done


# org and users are now synced from off2
#wait
# sync users and orgs
